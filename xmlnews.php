...
*/
	...
	if ($timeout > 0 || ini_get ("allow_url_fopen" ) == "0") { // als $timeout gezet is, of fopen wrappers niet geactiveerd zijn
		// breek de url op in delen $pu = parse_url( $url ); if ( isset( $pu['port'] ) ) { if ( $pu['port'] != 80 && $pu['port'] != 8080 ) $pu['port'] = 80; } else $pu['port'] = 80; // open socket (verbinding) met $timeout $errno = 0; $errstr = ""; if ( $timeout <= 0 ) $timeout = 3; $t = time(); $fp = fsockopen($pu['host'], $pu['port'], $errno, $errstr, $timeout); if (!$fp) return "Verbinding met gegevensbron mislukt..."; // vraag en lees $data uit socket met $timeout minus verstreken tijd $data = ""; $timeout -= (time() - $t); if ( $timeout <= 0 ) $timeout = 1; if ( function_exists( "stream_set_timeout" ) ) stream_set_timeout( $fp, $timeout ); elseif ( function_exists( "socket_set_timeout" ) ) socket_set_timeout( $fp, $timeout ); elseif ( function_exists( "set_socket_timeout" ) ) set_socket_timeout( $fp, $timeout ); fputs( $fp, "GET {$pu['path']} HTTP/1.0\r\nHost: {$pu['host']}:{$pu['port']}\r\n\r\n" ); while ( !feof( $fp ) ) $data .= fgets( $fp, 4096 ); fclose( $fp ); $data = substr( $data, strpos( $data, "\r\n\r\n" ) + 4 );
	...
	for ($i = 0; $i < count ( $node ); $i += 2) { // loop alle ITEM nodes af
		$item = array ();
		for ($j = $node [$i] + 1; $j < $node [$i + 1]; ++ $j) { // haal key=value data (zoals TITLE=x en LINK=y) uit deze ITEM node	
			$k = $values [$j] ["tag"];
			$v = $values [$j] ["value"];
			$item [$k] = $v;
		}
